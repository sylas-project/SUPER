...
#include "ros/ros.h"
#include "super_planner/PolynomialTrajectory.h"
#include "super_planner/PositionCommand.h"
...
class FsmRos1 : public Fsm {
  ...
  super_planner::PositionCommand pid_cmd_;
  rog_map::ROGMapROS::Ptr map_ptr_;
  super_planner::PositionCommand latest_cmd;
  nav_msgs::Path path;

  vector<super_planner::PositionCommand> cmd_logs_;
  ...
  void publishPolyTraj() override {
    super_planner::PolynomialTrajectory cmd_traj;
    getCommittedTrajectory(cmd_traj);
    mpc_cmd_pub_.publish(cmd_traj);
  }

  void getOneHeartBeatMsg(super_planner::PolynomialTrajectory &heartbeat,
                          bool &traj_finish) {
    heartbeat.type = super_planner::PolynomialTrajectory::HEART_BEAT;
  ...
  void getCommittedTrajectory(super_planner::PolynomialTrajectory &cmd_traj) {
    cmd_traj.header.stamp = ros::Time::now();
    cmd_traj.header.frame_id = "world";
    cmd_traj.type = super_planner::PolynomialTrajectory::POSITION_TRAJ |
                    super_planner::PolynomialTrajectory::HEART_BEAT;
    ...
    cmd_traj.coef_pos_x.resize(cmd_traj.piece_num_pos *
                               (cmd_traj.order_pos + 1));
    cmd_traj.coef_pos_y.resize(cmd_traj.piece_num_pos *
                               (cmd_traj.order_pos + 1));
    cmd_traj.coef_pos_z.resize(cmd_traj.piece_num_pos *
                               (cmd_traj.order_pos + 1));
    cmd_traj.start_WT_pos = ros::Time(pos_traj.start_WT);

    if (!yaw_traj.empty()) {
      cmd_traj.type =
          cmd_traj.type | super_planner::PolynomialTrajectory::YAW_TRAJ;
      ...
      for (int i = 0; i < cmd_traj.piece_num_yaw; i++) {
        ...
        Eigen::Map<Eigen::VectorXd>(&cmd_traj.coef_yaw[col_size * i],
                                    col_size) = yaw_coef;
  ...
  void getOnePositionCommand(super_planner::PositionCommand &pos_cmd,
                             bool &traj_finish) {
    pos_cmd.trajectory_flag = 0;
    StatePVAJ pvaj;
    double yaw, yaw_dot;
    bool on_backup_traj;
    planner_ptr_->getOneCommandFromTraj(pvaj, yaw, yaw_dot, on_backup_traj,
                                        traj_finish);
    ...
    Vec3f rpy, omg;
    double aT;
    geometry_utils::convertFlatOutputToAttAndOmg(pvaj.col(0), pvaj.col(1),
                                                 pvaj.col(2), pvaj.col(3), yaw,
                                                 yaw_dot, rpy, omg, aT);
...
public:
  ...
  void saveReplanLogToFile(const string &name = "") {
    ...
    fmt::print("Total replan num: {}, total length: {}, average computation "
               "time: {} ms\n",
               total_replan_num, total_length,
               average_compt_t /
                   (total_replan_num == 0 ? 1 : total_replan_num) * 1000);

    const std::string save_path =
        name.empty()
            ? LOG_FILE_DIR("replan_logs/" +
                           BinaryFileHandler<int>::getCurrentTimeStr() + ".bin")
            : LOG_FILE_DIR("replan_logs/" + name + ".bin");
    const std::string csv_path =
        name.empty()
            ? LOG_FILE_DIR("cmd_logs/" +
    ...
    csv_writer << "time,posi_x,posi_y,posi_z,vel_x,vel_y,vel_z,acc_x,acc_y,acc_"
                  "z,jerk_x,jerk_y,jerk_z,yaw,yaw_rate,backup"
               << std::endl;
    ...
    for (const auto &cmd : cmd_logs_) {
      csv_writer << cmd.header.stamp.toSec() - system_start_time_ << ","
                 ...
                 << cmd.position.z << "," << cmd.velocity.x << ","
                 << cmd.velocity.y << "," << cmd.velocity.z << ","
                 << cmd.acceleration.x << "," << cmd.acceleration.y << ","
                 << cmd.acceleration.z << "," << cmd.jerk.x << "," << cmd.jerk.y
                 << "," << cmd.jerk.z << "," << cmd.yaw << "," << cmd.yaw_dot
                 << "," << static_cast<int>(cmd.trajectory_flag) << std::endl;
    }
    csv_writer.close();
  }
  ...
  bool getPoseFromTraj(super_utils::Pose &pose) {
    if (machine_state_ != FOLLOW_TRAJ) {
      cout << YELLOW
           << "[Fsm] Not in FOLLOW_TRAJ state, can't get pose from traj."
           << RESET << endl;
      return false;
    }
    ...
    pose.first =
        Vec3f{pid_cmd_.position.x, pid_cmd_.position.y, pid_cmd_.position.z};
    pose.second = eulerToQuaternion(pid_cmd_.attitude.x, pid_cmd_.attitude.y,
                                    pid_cmd_.attitude.z);
    ...
    fmt::print(" -- [Fsm] Cur vel: {}, delta_v: {}, max_delta_v: {}\n",
               pid_cmd_.vel_norm, delta_v, max_delta_v);
    cmd_logs_.push_back(latest_cmd);
    return true;
  }

  void goalCallback(const geometry_msgs::PoseStampedConstPtr &msg) {
    super_utils::Vec3f goal_p =
        Vec3f{msg->pose.position.x, msg->pose.position.y, msg->pose.position.z};
    super_utils::Quatf goal_q =
        super_utils::Quatf{msg->pose.orientation.w, msg->pose.orientation.x,
  ...
  void init(const ros::NodeHandle &nh, const std::string &cfg_path) {
    ...
    cmd_pub = nh_.advertise<super_planner::PositionCommand>(cfg_.cmd_topic, 10);
    mpc_cmd_pub_ = nh_.advertise<super_planner::PolynomialTrajectory>(
        cfg_.mpc_cmd_topic, 10);
    ...
    if (cfg_.click_goal_en) {
      goal_sub_ =
          nh_.subscribe(cfg_.click_goal_topic, 1, &FsmRos1::goalCallback, this);
    ...
    if (cfg_.timer_en) {
      execution_timer_ = nh_.createTimer(
          ros::Duration(0.01), &FsmRos1::mainFsmTimerCallback, this); // 100Hz
      cmd_timer_ = nh_.createTimer(
          ros::Duration(0.01), &FsmRos1::pubCmdTimerCallback, this); // 100Hz
      replan_timer_ = nh_.createTimer(ros::Duration(1.0 / cfg_.replan_rate),
                                      &FsmRos1::replanTimerCallback,
                                      this); // 10Hz
    }

    write_time_.open(DEBUG_FILE_DIR("time_consuming.csv"),
  ...
  void pubCmdTimerCallback(const ros::TimerEvent &event) {
    ...
    super_planner::PolynomialTrajectory heartbeat;
  ...
  void mainFsmTimerCallback(const ros::TimerEvent &event) { callMainFsmOnce(); }
};
} // namespace fsm
