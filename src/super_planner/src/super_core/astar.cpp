...
Astar::Astar(const std::string &cfg_path,
  ...
  cout << rog_map::GREEN << " -- [RM] Init Astar-map." << rog_map::RESET
       << endl;
  int map_buffer_size =
  ...
  cout << rog_map::BLUE << "\tmap index size: " << cfg_.map_size_i.transpose()
       << rog_map::RESET << endl;
  cout << rog_map::BLUE << "\tmap vox_num: " << cfg_.map_voxel_num.transpose()
       << rog_map::RESET << endl;
  int test_num = 100;
...
Astar::setup(const Vec3f &start_pt, const Vec3f &goal_pt, const int &flag,
  ...
  if (md_.unknown_as_occ && md_.unknown_as_free) {
    cout << YELLOW << " -- [A*] " << RET_CODE_STR[INIT_ERROR]
         << ": cannot use both unknown_as_occupied and unknown_as_free."
         << RESET << endl;
    return INIT_ERROR;
  }
  if (md_.use_prob_map) {
...
void Astar::posToGlobalIndex(const rog_map::Vec3f &pos,
                             rog_map::Vec3i &id_g) const {
  if (md_.use_inf_map) {
...
void Astar::globalIndexToPos(const rog_map::Vec3i &id_g,
                             rog_map::Vec3f &pos) const {
  if (md_.use_inf_map) {
...
RET_CODE Astar::pointToPointPathSearch(const rog_map::Vec3f &start_pt,
  ...
  if (!insideLocalMap(start_pt)) {
    ...
    if (rog_map::lineIntersectBox(start_pt, md_.local_map_center_d,
      ...
      if (!map_ptr_->getNearestInfCellNot(OCCUPIED, local_start_pt,
        ...
        cout << rog_map::RED << " -- [A*] " << RET_CODE_STR[INIT_ERROR]
  ...
  if (!insideLocalMap(end_pt)) {
    rog_map::Vec3f seed_pt =
        start_pt_out_local_map ? md_.local_map_center_d : start_pt;
    if (rog_map::lineIntersectBox(end_pt, seed_pt, md_.local_map_min_d,
  ...
  if (!insideLocalMap(start_idx) || !insideLocalMap(end_idx)) {
    cout << rog_map::RED
         << " -- [RM] Start or end point is out of local map, which should not "
            "happen."
         << rog_map::RESET << endl;
  ...
  while (!open_set.empty()) {
      ...
      for (int dy = -1; dy <= 1; dy++)
        for (int dz = -1; dz <= 1; dz++) {
          ...
          if (neighborPtr == nullptr) {
            cout << rog_map::RED
                 << " -- [RM] neighborPtr is null, which should not happen."
                 << rog_map::RESET << endl;
            continue;
          }
    ...
    if (!cfg_.visual_process && (time_2 - time_1) > time_out) {
      fmt::print(fg(fmt::color::indian_red),
                 "Failed in A star path searching !!! {} seconds time limit "
                 "exceeded.\n",
                 time_out);
      return TIME_OUT;
    }
  ...
  if ((time_2 - time_1) > time_out) {
    fmt::print(fg(fmt::color::indian_red),
               "Time consume in A star path finding is {} s, iter={}.\n",
  ...
  if (md_.unknown_as_occ && !frontier_queue.empty()) {
    ...
    if (frontier_queue.empty()) {
      cout << rog_map::RED << " -- [A*] Frontier queue is empty, return."
           << rog_map::RESET << endl;
      return NO_PATH;
    }
    retrievePath(local_goal, node_path);
    ...
    ConvertNodePathToPointPath(node_path, out_path);
    cout << rog_map::BLUE << "Frontier queue: " << frontier_queue.size()
         << endl;
    return REACH_HORIZON;
  }
...
RET_CODE Astar::escapePathSearch(const rog_map::Vec3f &start_pt, const int flag,
  ...
  if (!insideLocalMap(start_pt) || !map_ptr_->insideLocalMap(start_pt)) {
    fmt::print(fg(fmt::color::indian_red),
               " -- [A*] {}: escape start point is not inside local map.\n",
  ...
  if (!map_ptr_->getNearestCellNot(OCCUPIED, start_pt, local_start_pt, 3.0)) {
    cout
        << rog_map::RED << " -- [A*] " << RET_CODE_STR[INIT_ERROR]
  ...
  cout << rog_map::RED
       << " -- [A*] Escape path searcher, cannot find path, return."
       << rog_map::RESET << endl;
  return NO_PATH;
}

bool Astar::neighborHaveOne(const rog_map::GridType &type,
                            const rog_map::Vec3i &src_id) {
